---
id: SPRINT-DIANA-ARACELY-ALVAREZ-VA
title: "Plan de Sprints — Diana Aracely Alvarez Varela"
owner: "Diana Aracely Alvarez Varela"
status: approved
version: "1.0"
traces_up: ["01_Product/PRD", "02_Requirements/User_Stories"]
traces_down: ["US-101", "US-102", "US-103", "US-104", "US-105", "US-106"]
last_reviewed: "2026-07-31"
tags: [sprint, plan, celula-1, nivel-alto]
---

# FARO · Plan de trabajo individual
## Diana Aracely Alvarez Varela

> **Proyecto:** FARO — Escuela como Sensor Social
> **Célula:** Celula 1 — Data Engineering & Quality · **Peso en rúbrica:** 2.5 pts
> **Rol:** Tech Lead · Data Engineering · **Nivel asignado:** Alto
...
| **Formato de entrega** | Rama `feat/diana-varela-...` → PR con plantilla completa → 2 aprobaciones → merge a `main` |
...
### `US-101` · Disenar el modelo de datos medallon completo

| | |
|---|---|
| **Sprint** | S1 — Lun 3 - Dom 9 ago |
| **Objetivo** | Contrato de las 3 capas: Bronze (raw + metadatos), Silver (tipado, CCT homologado) y Gold (esquema estrella). Incluye el parametro `SCOPE_ENTIDADES` y la politica de cobertura parcial. Documentar en `03_Architecture/Data_Model.md`. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/diana-varela-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-102` · Construir el DAG maestro de orquestacion en Airflow

| | |
|---|---|
| **Sprint** | S2 — Lun 10 - Dom 16 ago |
| **Objetivo** | DAG con dependencias, reintentos, alertas y particionado por fecha. Debe manejar 8 fuentes con periodicidad distinta: horaria, diaria, mensual, anual y censal. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/diana-varela-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-103` · Modelar la capa GOLD como esquema estrella

| | |
|---|---|
| **Sprint** | S3 — Lun 17 - Dom 23 ago |
| **Objetivo** | `fact_escuela_ciclo` + dim_escuela, dim_municipio, dim_tiempo, dim_driver. Es la pieza de mayor peso en la rubrica: debe quedar impecable. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/diana-varela-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-104` · Definir e implementar la tabla de features para ML

| | |
|---|---|
| **Sprint** | S3 — Lun 17 - Dom 23 ago |
| **Objetivo** | `gold.features_escuela`: los 6 drivers normalizados + banderas de cobertura. Contrato cerrado y versionado con la Celula 3. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/diana-varela-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-105` · Implementar la estrategia de cobertura parcial e indice de confianza

| | |
|---|---|
| **Sprint** | S3 — Lun 17 - Dom 23 ago |
| **Objetivo** | Interpolacion IDW para SINAICA dentro de radio valido; fuera de radio, marcar `SIN_DATO` explicito (nunca cero ni nulo silencioso). Calcular `indice_completitud_drivers` por escuela. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/diana-varela-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-106` · Congelar esquema y documentar linaje completo

| | |
|---|---|
| **Sprint** | S5 — Lun 31 ago - Dom 6 sep |
| **Objetivo** | Diagrama fuente->bronze->silver->gold->feature->modelo->dashboard. Freeze el 6 de septiembre. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/diana-varela-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |
...
| `US-106` | Congelar esquema y documentar linaje compl | ⬜ Por iniciar | 0% | — | Dom 6 sep |
