...
| **Objetivo** | Arquitectura de informacion de los 10 tableros, arbol de navegacion y definicion formal de cada KPI con su formula SQL. Documentar en `04_UX_Design/Screen_Specs.md`. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/manuel-reinada-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-202` · Configurar Superset: conexion, datasets y capa semantica

| | |
|---|---|
| **Sprint** | S3 — Lun 17 - Dom 23 ago |
| **Objetivo** | Conectar a Gold, datasets virtuales, metricas y dimensiones reutilizables por todos los tableros. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/manuel-reinada-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-203` · Construir DB-01 Ejecutivo y DB-02 Mapa de riesgo territorial

| | |
|---|---|
| **Sprint** | S4 — Lun 24 - Dom 30 ago |
| **Objetivo** | Los dos tableros insignia del pitch. DB-02 es coropletico municipal con puntos de escuela. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/manuel-reinada-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-204` · Construir DB-06 Predicciones y DB-09 Recomendaciones prescriptivas

| | |
|---|---|
| **Sprint** | S4 — Lun 24 - Dom 30 ago |
| **Objetivo** | Panel interactivo de simulacion sobre los 3 modelos y el tablero que dice que intervencion toca a cada escuela. DB-09 es el diferenciador del proyecto. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/manuel-reinada-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-205` · Integrar y armonizar los 10 dashboards

| | |
|---|---|
| **Sprint** | S5 — Lun 31 ago - Dom 6 sep |
| **Objetivo** | Estilo unificado, navegacion cruzada, tiempos de respuesta aceptables y revision de consistencia de cifras entre tableros. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/manuel-reinada-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |
