---
id: SPRINT-EMILIO-GALNARES-RUIZ
title: "Plan de Sprints — Emilio Galnares Ruiz"
...
| **Formato de entrega** | Rama `feat/emilio-ruiz-...` → PR con plantilla completa → 2 aprobaciones → merge a `main` |
...
### `US-121a` · Prueba de descarga real de DS-06 (CONAGUA) y DS-08 (CONAPO)

| | |
|---|---|
| **Sprint** | S1 — Lun 3 - Dom 9 ago |
| **Objetivo** | SOLO tus fuentes DS-06 y DS-08: descargar fisicamente, abrir, contar registros, verificar esquema y llave (clave INEGI 5 digitos) y llenar la prueba en `14_Data_Sources/DS-06_CONAGUA_SINA.md` y `DS-08_CONAPO_Proyecciones.md`. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/emilio-ruiz-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-122a` · Escribir los extractores de DS-06 (CONAGUA) y DS-08 (CONAPO)

| | |
|---|---|
| **Sprint** | S2 — Lun 10 - Dom 16 ago |
| **Objetivo** | Un extractor idempotente por fuente (DS-06 diaria, DS-08 anual) que deposite en Bronze como Parquet con `_ingested_at`, `_source`, `_source_url`. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/emilio-ruiz-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-123a` · Validaciones Great Expectations de DS-06 y DS-08

| | |
|---|---|
| **Sprint** | S3 — Lun 17 - Dom 23 ago |
| **Objetivo** | Suite GE para las tablas de DS-06 y DS-08: nulos, duplicados, rangos fisicos, tipos y llave. Data Docs publicables. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/emilio-ruiz-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-124a` · Fixtures de prueba anonimizados de DS-06 y DS-08

| | |
|---|---|
| **Sprint** | S4 — Lun 24 - Dom 30 ago |
| **Objetivo** | Muestra <=500 filas de DS-06 y DS-08 para que CI corra sin descargar datos reales. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/emilio-ruiz-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |
