...
| **Formato de entrega** | Rama `feat/andres-habib-...` → PR con plantilla completa → 2 aprobaciones → merge a `main` |
...
| **Objetivo** | Los 3 modelos, particion temporal (nunca aleatoria), protocolo de backtesting y manejo de cobertura parcial en features. Documentar como ADR. |
...
### `US-302` · Entrenar el Modelo 2 - Clasificacion de driver dominante

| | |
|---|---|
| **Sprint** | S4 — Lun 24 - Dom 30 ago |
| **Objetivo** | Multiclase: cual de los 6 drivers explica el riesgo de cada escuela. Con SHAP para explicabilidad. Es el corazon prescriptivo del proyecto. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/andres-habib-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |

### `US-303` · Registrar los 3 modelos en MLflow y exponerlos via API

| | |
|---|---|
| **Sprint** | S4 — Lun 24 - Dom 30 ago |
| **Objetivo** | Versionado, metricas, artefactos y contrato de inferencia batch y en tiempo real. |
| **Entregable** | Código en su carpeta + documento en el vault con frontmatter + fila en la matriz |
| **Cómo se entrega** | Rama `feat/andres-habib-...` → PR con plantilla → revisión del Tech Lead → merge a `main` |
