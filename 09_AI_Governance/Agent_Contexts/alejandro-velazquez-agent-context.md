...
> → [[09_AI_Governance/AI_Agent_Governance]] · Plan: [[12_Roadmap_Sprints/Sprints/5-alejandro-velazquez-mendoza]]
...
| ID | Sprint | Objetivo |
|---|---|---|
| US-521a | S1 | Documentar el setup local reproducible de FastAPI (API) y Postgres/Cloud SQL: variables, puertos y verificacion. |
| US-522a | S3 | Dockerfile y servicio en docker-compose para FastAPI y Postgres, con healthchecks. |
| US-523a | S3 | Configurar la proteccion de `main`: PR obligatorio, required reviews y prohibicion de push directo. |
| US-524a | S5 | Metricas, logs y alertas para FastAPI y Postgres. |
| US-525a | S6 | Procedimiento probado de rollback para FastAPI y Postgres. |

---

## 6. Reglas de uso de IA que aplican

- **DevLog obligatorio por sesión con IA**, antes del push (`_DevLog/YYYY-MM-DD-alejandro-velazquez-*.md`).
...
- **Nunca commit directo a `main`**: todo entra por PR (`feat/alejandro-mendoza-...`).
